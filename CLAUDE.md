...
| 2026-08-26 | 소셜(좋아요·댓글·피드 UI) | 피드 카드 개편(인라인 팔로우 버튼, 날짜를 닉네임 아래로) + 노트 좋아요/댓글 신설 | — | 서버: `noteLike`·`noteComment` 도메인 신설 — `POST/DELETE /tasting-note/{id}/like`(멱등), `GET/POST/DELETE /tasting-note/{id}/comments`(300자, 차단 관계 양방향 숨김, 삭제권=본인 댓글 또는 노트 주인, COMMENT4001/4002/4041). 피드·상세 응답에 `isFollowing`(**Boolean 래퍼 필수** — primitive boolean이면 Lombok+Jackson이 JSON 키를 `following`으로 바꿈)·`likeCount`·`likedByMe`·`commentCount` 추가(피드는 IN 배치 쿼리 4개로 N+1 방지). `ReportType.COMMENT` 추가. FK 정리: 탈퇴(`deleteByMemberId`)·노트 삭제·와인 등록 거절 경로에 좋아요/댓글 삭제 추가 — **거절 경로가 노트 사진 row를 안 지우던 기존 FK 구멍(`deleteImagesByWineId`)도 함께 수정**(벌크 JPQL은 cascade를 안 탐). 클라: FeedCard 작성자 행 재구성(아바타 34px+닉네임/날짜 2줄, 우측 팔로우 버튼 — `isFollowing===false && !mine`일 때만), 사진 아래 액션 행(하트 낙관적 토글+서버 카운트 정본, 댓글 수→상세 이동), `appEvents` 3종 신설(memberFollowChanged·noteLikeChanged·noteCommentCountChanged)로 피드↔상세↔프로필 동기화, 상세 화면에 좋아요 행+댓글 카드(v1 전체 로드 최대 100, 입력/삭제/신고 — 신고는 기존 `POST /report`에 COMMENT 타입), 구서버 호환은 소셜 필드 undefined→미노출 폴백. **잔여**: 댓글 100개 초과 페이지네이션, 홈 피드 섹션 티저 카드에는 좋아요 미표시, 실기기/시뮬레이터 확인 |
